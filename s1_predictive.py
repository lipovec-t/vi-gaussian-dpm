...
for i in tqdm(range(MC_runs), desc='(1/8) Uniform Init '):
    ...
    _, _, predictive[i, :], predictive_converged_it[i], _, _, _ = coordinates_ascent(data_dict, params)

# average results
predictive_avg = np.mean(predictive, axis=0)
predictive_converged_it_avg = np.mean(predictive_converged_it)
# 95% confidence interval
(ci_min1, ci_max1) = st.t.interval(confidence = 0.95, df = predictive.shape[0]-1,\
                                loc = predictive_avg,\
                                scale = st.sem(predictive, axis=0))
(ci_min2, ci_max2) = st.t.interval(confidence = 0.95, df = predictive_converged_it.shape[0]-1,\
                                loc = predictive_converged_it_avg,\
                                scale = st.sem(predictive_converged_it, axis=0))
    
ax2.plot(it_counter, predictive_avg, color='b', label="Uniform")
ax2.fill_between(it_counter, ci_min1, ci_max1, color='b', alpha=.1)
ax2.axvline(x=predictive_converged_it_avg, color='b', linestyle='--')
...
ax1.plot(it_counter, predictive_avg, color='g', label="True")
ax1.fill_between(it_counter, ci_min1, ci_max1, color='g', alpha=.1)
ax1.axvline(x=predictive_converged_it_avg, color='g', linestyle='--')
ax1.axvspan(ci_min2, ci_max2, alpha=0.1, color='g')
...
ax1.plot(it_counter, predictive_avg, color='r', label="Random")
ax1.fill_between(it_counter, ci_min1, ci_max1, color='r', alpha=.1)
ax1.axvline(x=predictive_converged_it_avg, color='r', linestyle='--')
...
ax1.plot(it_counter, predictive_avg, color='c', label="Unique")
ax1.fill_between(it_counter, ci_min1, ci_max1, color='c', alpha=.1)
ax1.axvline(x=predictive_converged_it_avg, color='c', linestyle='--')
...
ax2.plot(it_counter, predictive_avg, color='k', label="DBSCAN")
ax2.fill_between(it_counter, ci_min1, ci_max1, color='k', alpha=.1)
ax2.axvline(x=predictive_converged_it_avg, color='k', linestyle='--')
...
ax2.plot(it_counter, predictive_avg, color='brown', label="Global")
ax2.fill_between(it_counter, ci_min1, ci_max1, color='brown', alpha=.1)
ax2.axvline(x=predictive_converged_it_avg, color='brown', linestyle='--')
