...
# ELBO calculation
def compute_elbo(alpha, lamda, x, gamma, phi, tau, sigma_U, mu_G, sigma_G):
    # number of clusters
    T = phi.shape[1]
    # data dimension
    K = x.shape[1]
    ...
    expec_eta, expec_logpart_eta    = expec.eta_and_logparteta(tau, sigma_U)
    ...
    h = 1 / ((2*np.pi/lamda[-1])**(K/2)*np.linalg.det(np.linalg.inv(sigma_U))**0.5)
    a = np.dot(lamda[:-1], np.matmul(np.linalg.inv(sigma_U), lamda[:-1])) / (2*lamda[-1])
    ...
    D_1 = -0.5*np.sum(x*np.dot(np.linalg.inv(sigma_U), x.T).T, axis=1) - np.repeat(const, N, axis=0) 
    ...
    # Term F
    h = 1 / ((2*np.pi/tau[:,-1])**(K/2)*np.linalg.det(np.linalg.inv(sigma_U))**0.5)
    a = np.sum(tau[:,:-1]*np.dot(np.linalg.inv(sigma_U), tau[:,:-1].T).T, axis=1) / (2*tau[:,-1])
